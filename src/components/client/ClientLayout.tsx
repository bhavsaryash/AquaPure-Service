...
const ClientLayout: React.FC<ClientLayoutProps> = ({ children }) => {
  ...
  const navigation = [
    ...
    { name: 'Service History', href: '/client/services', icon: History },
    { name: 'Request Service', href: '/client/request', icon: Plus },
    { name: 'My AMC', href: '/client/amc', icon: Droplets },
            ...
            <nav className="mt-5 flex-1 px-2 bg-white space-y-1">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`mr-3 h-5 w-5 ${isActive ? 'text-green-500' : 'text-gray-400 group-hover:text-gray-500'}`} />
                    {item.name}
                  </Link>
                );
              })}
